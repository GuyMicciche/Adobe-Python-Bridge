...
void AEPython::showScriptLibrary()
{
	exec(u8R"(
import qtae
...
void AEPython::togglePythonWindow()
{
	exec(u8R"(
import qtae
qtae.TogglePythonWindow()
)");
}

void AEPython::toggleScriptLibrary()
{
	exec(u8R"(
import qtae
import script_library
...
bool AEPython::isPythonWindowVisible()
{
	try
	{
		// qtae is already imported during init, just call the function directly
		py::object qtae = py::module_::import("qtae");
		py::object result = qtae.attr("IsPythonWindowVisible")();
...
bool AEPython::isScriptLibraryVisible()
{
	try
	{
		// script_library should be imported
		py::object script_library = py::module_::import("script_library");
		py::object result = script_library.attr("IsScriptLibraryVisible")();
		return result.cast<bool>();
	}
	catch (py::error_already_set&)
	{
